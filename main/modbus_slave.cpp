...
static void log_hex_i [[maybe_unused]] (const char *tag, const char *label, const uint8_t *data, int len)
{
    if (!data || len <= 0)
        return;
                                                        ...
                                                        int *out_frame_len)
{
    ...
    ESP_LOGD(TAG, "parse_modbus_request_frame: buf[0]=0x%02X buf[1]=0x%02X len=%d", buf[0], buf[1], len);
...
static bool is_echoed_modbus_response(const uint8_t *buf, int len, uint8_t unit_id, int *out_len)
{
    ...
    if (func == 0x06)
    {
        ...
        if (s_last_tx_len != 8)
            return false;
        if (memcmp(buf, s_last_tx, 8) != 0)
            return false;
        const int64_t now_us = esp_timer_get_time();
        if (now_us - s_last_tx_us > 5000)
            return false; // outside typical echo window
        *out_len = 8;
        return true;
    }
...
static bool uart_write_bytes_blocking(uart_port_t port, const uint8_t *data, size_t len, TickType_t ticks)
{
    ...
    size_t copy_len = len;
    if (copy_len > sizeof(s_last_tx))
        copy_len = sizeof(s_last_tx);
    if (data && copy_len > 0)
    {
            ...
            for (int offset = 0; offset < idx;)
            {
                ...
                if (slave == MODBUS_UNIT_ID)
                {
                    ...
                    else if (func == 0x10)
                    {
                        ...
                        // ESP_LOGI(TAG, "Write Multiple Registers: addr=%u count=%u", addr, count);
                        // Check if writing to RTC time registers (addr 200-205: year, month, day, hour, minute, second)
                        if (addr >= 200 && addr <= 205 && count <= 6)
                        {
                            // Extract time values from data
                            uint16_t year = (uint16_t)((data[0] << 8) | data[1]);
                            uint16_t month = (uint16_t)((data[2] << 8) | data[3]);
                            uint16_t day = (uint16_t)((data[4] << 8) | data[5]);
                            uint16_t hour = (count > 2) ? (uint16_t)((data[6] << 8) | data[7]) : 0;
                            uint16_t minute = (count > 3) ? (uint16_t)((data[8] << 8) | data[9]) : 0;
                            uint16_t second = (count > 4) ? (uint16_t)((data[10] << 8) | data[11]) : 0;

                            ESP_LOGI(TAG, "RTC Set: %04u-%02u-%02u %02u:%02u:%02u", year, month, day, hour, minute, second);
                            // TODO: Call your RTC set function here
                            app.set_rtc_time(year, month, day, hour, minute, second);
                            ESP_LOGI(TAG, "RTC Set confirmed: %04u-%02u-%02u %02u:%02u:%02u", year, month, day, hour, minute, second);
                        }
                        else
                        {
